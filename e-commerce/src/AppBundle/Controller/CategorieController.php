...
class CategorieController extends Controller
{
    ...
    public function newAction(Request $request, LoggerInterface $logger)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try{
                $image->move(
                    $this->getParameter('uploads_directory_cat'),
                    $fileName
                );
            }
            ...
            $logger->info($categorie->__toString());
            $slug = $this->get('make_slug')->makeSlug($categorie->getLibelle());
     ...
     *
     * @Route("del/{id}", name="admin_categories_delete")
     * @Method("DELETE")
     */
